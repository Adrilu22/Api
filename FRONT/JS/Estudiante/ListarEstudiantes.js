async function listarTodos() {
    ...
    try {
        ...
        if (respuesta && respuesta.length > 0) {
            ...
            titulo.innerHTML = "No se encontraron estudiantes.";
        }
    } catch (error) {
...
async function getData(url) {
    try {
        ...
        if (!response.ok) {
            throw new Error("Hubo un problema al obtener los datos de los estudiantes.");
        }
...
function generarLinkDescarga(data) {
    ...
    downloadLink.setAttribute("download", "estudiantes.json");
